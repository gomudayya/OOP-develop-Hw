...
public class ChatServer {
	...
	private static ChatServer uniqueInstance = null; //singleton
	private ChatServer() {}
	...
	public void addUserToRoom(String userID, String roomName) {
		// 예외 처리 추가
		// 1. 룸네임에 대응하는 룸이 존재하지 않음
		// 2. 룸네임에 해당하는 룸이 존재하지만, 그 룸에 이미 같은 UserID가 있음. //-> chatRoom.addUser에서 처리
		if(!chatRooms.containsKey(Objects.requireNonNull(roomName)))
			throw new IllegalArgumentException("해당 Room이 존재하지 않습니다.");
	...
	public void deleteUserFromRoom(String userID, String roomName) {
		// 예외 처리 추가
		// 1. 룸네임에 해당하는 룸이 존재하지 않음.
		// 2. 룸네임에 해당하는 룸이 존재하지만, 그 룸 안에 uesrID라는 유저가 존재하지 않음 
		//     -> chatRoom.deleteUser 에서 처리
	...
	public void deleteMessage(String roomName, int index) {
		// 완성하시오.
		chatRooms.get(roomName).deleteMessage(index); // 서버에 영향을 주어요..
	}
